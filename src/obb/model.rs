...
use super::{BBox, Config, Prediction, network};

/// YOLO26 oriented bounding-box detection model.
pub struct Model {
    net: network::Network,
    device: Device,
...
impl Model {
    /// Loads a YOLO26 OBB model from SafeTensors bytes.
    ...
    /// Loads an OBB model from a `.safetensors` file.
    ...
    /// Loads a YOLO26 OBB model directly from an official `.pt` checkpoint.
    #[cfg(feature = "pt")]
    ...
    /// Loads an OBB model from a `.pt` or `.safetensors` checkpoint, inferring
    /// `scale`, `labels_count`, `device` and `dtype` automatically.
    ...
    pub fn from_file_with(path: impl AsRef<std::path::Path>, mut config: Config) -> Result<Self> {
        ...
        config.labels_count =
            crate::model::infer_labels_count_from_shapes(crate::model::InferredTask::Obb, &shapes)?;
    ...
    /// Loads an OBB model from an in-memory `.pt` or `.safetensors` byte
    /// buffer, inferring `scale`/`labels_count` automatically.
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        Self::from_bytes_with(bytes, Config::default())
    }

    /// Loads an OBB model from an in-memory byte buffer, inferring
    /// `scale`/`labels_count` while honoring config overrides.
    ...
    /// Loads a YOLO26 OBB model directly from an in-memory `.pt` checkpoint
    ...
    /// Runs oriented bounding-box detection for one image.
...
fn postprocess_obb(
    ...
    let (rows, flattened) = flattened_rows(output, 7)?;
    let mut detections = Vec::new();

    for row in 0..rows {
        let r = OutputViewer::for_obb(&flattened, row).ok_or_else(|| {
            crate::Error::InvalidTensor(format!("obb output row {row} out of range"))
        })?;
        ...
        let bbox = BBox {
            center_x: letterbox.to_source_x(r.cx()),
            center_y: letterbox.to_source_y(r.cy()),
            width: letterbox.to_source_size(r.w()),
            height: letterbox.to_source_size(r.h()),
            angle: unsafe { r.angle() },
        };
        if bbox.width <= 0.0
            || bbox.height <= 0.0
            || bbox
