//! Cross-image visual-prompt encoding for the YOLOE segment model.
//!
...
impl Model {
    ...
    /// ([`VisualSource::Masks`]). One encoded table can be reused across
    /// many target images; the class set is fixed by the `prompts` class ids.
...
pub(crate) fn require_savpe(savpe: Option<&Encoder>) -> crate::Result<&Encoder> {
    savpe.ok_or_else(|| {
        crate::Error::InvalidConfig(
            "YOLOE visual-prompt inference requires official SAVPE weights".to_string(),
        )
    })
}
