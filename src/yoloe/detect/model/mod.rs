...
use crate::yoloe::detect::model::config::Config as ModelConfig;
use crate::yoloe::detect::model::network::Network;
use crate::yoloe::reprta::RepRta;

/// YOLOE open-vocabulary detection-only model backed by SafeTensors weights.
///
/// This is the detect-only sibling of
/// [`Model`](super::Model): it wraps
/// the backbone, neck, and [`Head`](super::Head)
/// (boxes + prompt class logits, no mask/prototype branches) and exposes the
/// same forward/predict surface, returning detection tensors without
...
pub struct Model {
    ...
    /// time; `None` when the checkpoint has no RepRTA. Borrowed by
    /// [`Session::text`](super::Session::text).
    pub(crate) reprta: Option<RepRta>,
}

impl Model {
    /// Loads a YOLOE open-vocabulary detection model from SafeTensors bytes,
    ...
    /// Loads a YOLOE detection model from a `.pt` or `.safetensors` checkpoint,
    /// inferring `scale`, `device`, `dtype` and the head layout automatically.
    ...
    /// Loads a YOLOE detection model from an in-memory `.pt` or `.safetensors`
    /// byte buffer, inferring `scale`, `device`, `dtype` and the head layout
    /// automatically.
    ...
    /// Loads a YOLOE open-vocabulary detection model directly from an official
    /// `.pt` checkpoint, inferring the head layout from it.
    #[cfg(feature = "pt")]
    ...
    /// Loads a YOLOE detection model directly from an in-memory `.pt` byte
    /// buffer (no filesystem). The `pt` feature is required.
    #[cfg(feature = "pt")]
